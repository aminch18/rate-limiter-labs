...
func TestFixedWindow_Allow(t *testing.T) {
	tests := []struct {
		...
		{"single request limit", 1, 2, 1},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			rl := fixedwindow.New(tc.limit, 60) // long window so it never resets
			allowed := 0
...
func TestFixedWindow_AllowN(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			rl := fixedwindow.New(tc.limit, 60)
			got := rl.AllowN(tc.n)
			if got != tc.want {
...
func TestFixedWindow_WindowExpiry(t *testing.T) {
	rl := fixedwindow.New(2, 1) // 1-second window
	rl.Allow()
	rl.Allow()
	if rl.Allow() {
		t.Fatal("expected rejection when limit reached")
	}
...
func TestFixedWindow_Remaining(t *testing.T) {
	rl := fixedwindow.New(5, 60)
	if rem := rl.Remaining(); rem != 5 {
		t.Errorf("fresh window remaining = %d, want 5", rem)
	}
	rl.Allow()
	if rem := rl.Remaining(); rem != 4 {
		t.Errorf("after 1 allow remaining = %d, want 4", rem)
	}
	rl.Reset()
	if rem := rl.Remaining(); rem != 5 {
